        ...
        for j in range(ntype):
            ...
            if i == j:
                ...
                if adj_list[i][j].nnz == 0:
                    # continue
                    feature_i_j = tf.zeros(
                        [feature_list[i].shape[0], args.embed_size], dtype=tf.float32
                    )
                else:
                    adj = _convert_sp_mat_to_sp_tensor(adj_list[i][j])
                    feature_i_j = self_attn(
                        args, feature_list[i], feature_list[j], adj, _name_scope
                    )
...
def seq_attn(args, feature1, seq_item, pos_emb, mask):
    with tf.variable_scope("sequential_aware_attention", reuse=tf.AUTO_REUSE):
        ...
        key_masks = tf.tile(
            tf.expand_dims(key_masks, 1), [1, tf.shape(seq)[1], 1]
        )  # (h*N, T_q, T_k)
        ...
        query_masks = tf.tile(
            tf.expand_dims(query_masks, -1), [1, 1, tf.shape(seq)[1]]
        )  # (h*N, T_q, T_k)
        ...
        # Restore shape
        h_dynamic = tf.reduce_sum(
            tf.concat(tf.split(outputs, args.n_head, axis=0), axis=2), 1
        )  # (N, C)
        ...
        coef = tf.reshape(
            tf.squeeze(tf.matmul(tf.reshape(s_feature2, [-1, M]), w_static)),
            [-1, args.embed_size],
        )  # [N,F]
...
def second_attn(args, inputs, name_scope):
    with tf.variable_scope(name_scope, reuse=tf.AUTO_REUSE):
        ...
        w_omega = tf.get_variable(
            "w_omega", initializer=tf.random_normal([args.embed_size, 1], stddev=0.1)
        )
        b_omega = tf.get_variable(
            "b_omega", initializer=tf.random_normal([1], stddev=0.1)
        )
        u_omega = tf.get_variable(
            "o_omega", initializer=tf.random_normal([1], stddev=0.1)
        )
